...
from homm1.symbols.annotated_compgen_data import _cache_key, source_compgen_data
...
class AnnotatedCompgenDataTest(unittest.TestCase):
    def test_cache_key_tracks_shared_annotated_data_parser(self):
        with TemporaryDirectory() as directory:
            ...
            def key(shared):
                ...
                with (mock.patch(
                        "homm1.symbols.annotated_compgen_data.Path.read_bytes",
                        autospec=True, side_effect=content),
                      mock.patch(
                          "homm1.symbols.annotated_compgen_data._source_dependencies",
                          return_value=[])):
    ...
    def test_clang_derives_literal_kind_and_logical_size(self):
        with TemporaryDirectory() as directory:
            ...
            rows = source_compgen_data(Path(directory), Path(directory))
    ...
    def test_rejects_duplicate_semantic_identity(self):
        with TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(ValueError, "duplicate DATA_COMPGEN semantic"):
                source_compgen_data(Path(directory), Path(directory))
    ...
    def test_clang_infers_compiler_float_from_integer_expression_context(self):
        with TemporaryDirectory() as directory:
            ...
            include.mkdir()
            (include / 'va.h').write_text('#define DATA_COMPGEN(address, name, value) (value)\n')
            source = Path(directory) / "SOURCE"
            source.mkdir()
