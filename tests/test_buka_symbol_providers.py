...
from homm1.symbols.fixed_asm import claims as fixed_asm_claims, unit as fixed_asm_unit
...
class FixedAsmTest(unittest.TestCase):
    def test_buka_fixed_masm_addresses_are_not_homm1_claims(self):
        self.assertEqual(list(fixed_asm_claims()), [])
        self.assertIsNone(fixed_asm_unit("BASE/BITS"))
...
class CompilerDataProviderTest(unittest.TestCase):
    ...
    def test_current_common_emission_authorizes_the_reviewed_identity(self):
        with mock.patch(
            "homm1.symbols.symbol_providers.CoffObject", return_value=self.coff(),
        ):
    ...
    def test_missing_common_emission_fails_closed(self):
        with mock.patch(
            "homm1.symbols.symbol_providers.CoffObject",
    ...
    def test_wrong_common_size_fails_closed(self):
        with mock.patch(
            "homm1.symbols.symbol_providers.CoffObject", return_value=self.coff(size=4),
        ):
