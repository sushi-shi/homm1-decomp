...
from homm1.audit import readability
from homm1.audit.readability import (
...
class ReadabilityTests(unittest.TestCase):
    ...
    def test_unexpanded_project_macro_fails(self):
        with self.assertRaisesRegex(ValueError, "Unexpanded"):
            tag_rows([dict(path="a.cpp", line=1, end=1, name="H1_ENUM_RETURN",
...
class InventoryOutputTests(unittest.TestCase):
    def setUp(self):
        ...
        self.reviews = self.root / 'config/cleanliness/file_reviews.json'
...
CTAGS = os.environ.get("HOMM1_READABILITY_CTAGS") or shutil.which("ctags")
...
''')
    ...
    def test_project_macros_do_not_hide_constructor_or_method(self):
        rows = self.index("""H1_ENUM_BEGIN(Kind)
    A = 0
H1_ENUM_END(Kind)
RVA(0x00400001, 1)
Thing::Thing() {}
H1_ENUM_RETURN(Result, i16) Thing::Main() { return 0; }
""")
        self.assertEqual([r["name"] for r in rows], ["Thing::Thing", "Thing::Main"])

    def test_unused_wrapper_preserves_all_three_helpers(self):
        rows = self.index("""static inline i16 H1_UNUSED(Width)(bitmap* p) { return p->w; }
static inline u8* H1_UNUSED(Pixels)(bitmap* p) { return p->pixels; }
static inline u8* H1_UNUSED(Pixels)(bitmap* p, i32 n) { return p->pixels + n; }
""")
        self.assertEqual([r["name"] for r in rows], ["Width", "Pixels", "Pixels"])

    def test_constraints_defaulted_bodies_and_final_owner(self):
        rows = self.index("""class Thing H1_FINAL {
public:
    Thing() = default;
