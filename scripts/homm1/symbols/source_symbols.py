#!/usr/bin/env python3
"""Buka source-symbol scanner, adapted to HoMM1 RVA annotations.

The matching graph uses symbols_for_file and render; its joined inventory is
written separately to build/gen/symbol_names.csv. Binary providers default off
here because HoMM1 admits sparse reviewed references through model.resolve.
Data-scanner dependencies supply identities only, never data matching credit.
...
Unlike homm1.symbols.annotated_functions - a library for the static-helper case -
this names every annotated definition, free function and method alike, since
nothing else will.

    python3 -m homm1.symbols.source_symbols            # -> build/gen/source_symbols.csv
    python3 -m homm1.symbols.source_symbols --check    # report, write nothing
"""
...
from homm1.symbols.annotated_data import (
    DATA_TOKEN, ClangMode, _clang_args, definitions_for_file,
)
from homm1.symbols.annotated_compgen_data import (
    compgen_data_symbol_name,
    source_compgen_data,
)
from homm1.symbols.annotated_vtables import source_vtables
from homm1.symbols.symbol_providers import (
    assembly_claims,
...
OUTPUT = REPO / "build/gen/source_symbols.csv"
...
def symbols_for_file(path: Path, source_root: Path, repo: Path,
    ...
    translation = (index or ci.Index.create()).parse(
        str(path), args=args if args is not None else _clang_args(repo, path, mode=ClangMode.RETAIL_ANALYSIS))
    # HoMM1 requires a complete AST under its measured analysis profile.
    errors = [d for d in translation.diagnostics if d.severity >= ci.Diagnostic.Error]
    own = errors  # HoMM1's measured SDK profile must parse without recovery.
    if own:
    ...
    for cursor in translation.cursor.walk_preorder():
        ...
        if bindings is not None:
            bindings[cursor.location.offset] = (
                rows[-1], 'internal' if cursor.linkage == ci.LinkageKind.INTERNAL else 'external')
...
def _vc6_symbol_name(cursor) -> str:
    ...
    if cursor.kind == ci.CursorKind.DESTRUCTOR and name.startswith("??_D"):
        access = {
            ci.AccessSpecifier.PUBLIC: ("Q", "U"),
            ci.AccessSpecifier.PROTECTED: ("I", "M"),
            ci.AccessSpecifier.PRIVATE: ("A", "E"),
        }[cursor.access_specifier][cursor.is_virtual_method()]
        if not name.endswith("@@QAEXXZ"):
            raise ValueError(f"unsupported destructor mangling; probe the period compiler: {name}")
...
REVIEWED_CLAIMS = (
    ("config/retail/compgen_functions.csv", None,
     lambda row: (row["unit"], row["symbol"])),
    ("config/retail/crt_functions.csv", "reviewed-crt",
     lambda row: ("(crt)", row["symbol"])),
    ("config/retail/import_thunks.csv", "reviewed-thunk",
     lambda row: ("(imports)", (row.get("coff") or "").strip() or "%s@%s" % (
         row["symbol"], row["dll"].rsplit(".", 1)[0]))),
    ("config/retail/eh_funclets.csv", "reviewed-funclet",
...
def collect(source_root: Path, repo: Path,
    ...
    if include_binary_providers is None:
        include_binary_providers = (
            False  # Sparse HoMM1 joins only admitted providers in model.resolve.
        )
    if include_binary_providers:
        provider_rows = assembly_claims()
        provider_rows += import_claims(
            repo / "build/orig/HEROES.EXE",
            repo / "build/objdiff/base",
            repo / "build/toolchains/vc40/lib",
        )
        provider_rows += compiler_data_claims(
            repo / "config/retail/compiler_generated_data.tsv",
            repo / "build/objdiff/base",
        )
...
def _manifest_targets(repo: Path) -> list[int]:
    ...
    manifest = repo / "config/retail/delink_relocs.tsv"
    exe = repo / "build/orig/HEROES.EXE"
