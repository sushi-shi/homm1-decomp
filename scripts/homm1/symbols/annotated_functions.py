...
VA_TOKEN = re.compile(rb"\bRVA\s*\(")
VA_MARKER = re.compile(
    rb"^[ \t]*(?:extern[ \t]+)?RVA\(\s*(0x[0-9a-fA-F]+)\s*,\s*(0x[0-9a-fA-F]+|[0-9]+)\s*\)",
    re.M)
STATIC_VA_TOKEN = re.compile(
    rb"\bRVA\s*\([^)]*\)\s*(?:(?!\{).){0,1024}\bstatic\b", re.S)
VA_ANNOTATION = re.compile(r"^rva:(0x[0-9a-fA-F]+) size:(0x[0-9a-fA-F]+|[0-9]+)$")
...
def _annotation(cursor) -> tuple[int, int] | None:
    ...
    return IMAGE_BASE + int(matches[0].group(1), 16), int(matches[0].group(2), 0)
...
def source_function_spans(source_root: Path,
    ...
    for path in sorted(source_root.rglob("*.cpp")):
        ...
        for match in VA_MARKER.finditer(masked):
            va = IMAGE_BASE + int(match.group(1), 16)
