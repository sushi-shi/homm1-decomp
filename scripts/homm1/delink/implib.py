"""homm1.delink.implib - PROVEN `__imp_` decorations for the IAT.
...
def resolve_iat(slots, base_dir: Path | None) -> tuple[list, list]:
    ...
    for slot, name, dll, ordinal in slots:
        ...
        if name is None:
            ...
            # Some HoMM1 vendor DLLs expose the complete x86-decorated name in
            # the PE import table itself (for example `_AIL_serve@0`).  That is
            # already exact evidence; no import library or @N inference is
        ...
        # Vostok can partition every relocation; do not invent an API name or
        # an @N stack size.  A real import library supersedes these fallbacks
        # automatically through the proven branches above.
        if dec is None and name is None:
            safe_dll = re.sub(r"[^A-Za-z0-9_]", "_", dll.lower())
            dec = f"__imp_HOMM1_ORD_{safe_dll}_{ordinal}"
        elif dec is None:
            dec = "__imp_" + name
        if dec:
