"""homm1.verify.data_coverage - is every datum MODELLED, not merely byte-equal?
...
range (`touched`/`sites`, from homm1.verify.data_access's sweep).
...
VERDICTS
  ...
  OVERLAP      two claims covering one byte with different extents (a folded
    ...
    python3 -m homm1.verify.data_coverage [--verdict V] [--near RVA]
    python3 -m homm1.verify.data_coverage --tsv | --overlaps | --gate
"""
...
def load_claims(path=MANIFEST):
    ...
    if not path.is_file():
        raise SystemExit(f"no {path} - run `homm1 build` first")
...
def main(argv=None) -> int:
    ...
    if a.tsv:
        changed = write_tsv(
            a.tsv, ["# GENERATED by homm1.verify.data_coverage - retail bytes "
