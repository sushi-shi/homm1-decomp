"""homm1.verify.data_relocs - the data-side wrong-referent sieve (full).
...
opens - both silent-unscored classes, both FATAL.
    ...
    python3 -m homm1.verify.data_relocs [--unit U] [--calibrate] [--gate]
"""
...
def gate_findings() -> list[str]:
    ...
    if not sum(stats.values()):
        ...
        out.append("data-relocs: 0 base/target pairs scanned - the "
                   ...
                   "compared at all. Run `homm1 build` and re-run.")
...
def main(argv=None) -> int:
    import argparse
    ap = argparse.ArgumentParser(prog="homm1 verify data-relocs",
                                 description=__doc__,
