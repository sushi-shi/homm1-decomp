"""homm1.verify.data_access - the RETAIL-side data-access audit (full tier).
...
Engine + schema: homm1.verify.access_map (retail bytes and .reloc from
...
carry the measured rate and the reason, one line each.
    ...
    python3 -m homm1.verify.data_access --at 0x24e360 | --symbol NAME
    python3 -m homm1.verify.data_access --fn NAME | --sql "SELECT ..."
    python3 -m homm1.verify.data_access --touched       the byte ranges retail
...
REPORT_ONLY = {
    ...
    "unaccessed": "phantom CANDIDATES only - a dead-but-correct datum "
                  "(g_unreferencedHoMM1MgrValues' bytes ARE retail's) and a library global "
...
def analyse(trace=None):
    ...
    ride a stale map."""
    from homm1.verify.access_map import Owners
...
def do_touched(args):
    ...
    write_tsv(args.touched,
              ["# GENERATED by homm1.verify.data_access --touched - the byte "
