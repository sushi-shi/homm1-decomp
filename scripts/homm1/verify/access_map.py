"""homm1.verify.access_map - THE static map of every data access retail makes.
...
PLUMBING (the port): retail bytes + relocations from homm1.sema.image, the
decode from homm1.tool.objdump, the claim spine from homm1.model (which
replaced symbol_names.csv), and the declared field layout from
homm1.verify.layout (pylibclang under the TU's own i386/MSVC flags).
"""
...
def section_pct():
    ...
    import json
    from homm1.verify import scores as sc
    out = {}
    try:
...
def persist(img, layout, accesses, cells, claims, stats, sqlite_path=SQLITE,
    ...
    write_tsv(tsv_path, ["# GENERATED by homm1.verify.access_map - every "
...
def connect(path=SQLITE):
    if not Path(path).exists():
        raise SystemExit(f"no access map at {path} - run "
                         f"`python3 -m homm1.verify.data_access --build`")
...
def load():
    ...
    from homm1.verify.layout import harvest
    img = retail()
    model = resolve()
