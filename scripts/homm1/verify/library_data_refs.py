...
reported by ``homm1 verify data-coverage --tsv``.

    homm1 verify library-data-refs
    homm1 verify library-data-refs --near 0x1eafbc
    homm1 verify library-data-refs --tsv
"""
...
def main(argv=None) -> int:
    ...
    for required in (GAPS_TSV, ACCESS_TSV):
        if not required.is_file():
            ap.error(f"missing {required}; run homm1 verify data-coverage --tsv")
