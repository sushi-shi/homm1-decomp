"""homm1.tool.objdump - the raw-binary i386 disassembler.

    homm1 tool objdump <file.bin> [--vma 0x153810]
...
which function - is the caller's business (homm1.sema.disasm).
...
def disassemble(blob: bytes, vma: int = 0, *, arch: str = "i386",
    ...
    if shutil.which(exe) is None:
        raise ToolError(f"{exe} not found on PATH - run inside `nix develop`")
    with tempfile.TemporaryDirectory(prefix="homm1-objdump-") as tmp:
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap = argparse.ArgumentParser(
        prog="homm1 tool objdump", description=__doc__,
