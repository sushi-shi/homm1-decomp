# Copied from HoMM2 Gold 2.1 Buka at 299514f88900c0cf30ba03422c72830a38fc1cb7.
# Local adaptation: command name in diagnostics only.
...
accepted; regenerating through ``homm1 build`` writes current stamps.
...
def freshness_problems(output: Path, _seen: set | None = None) -> list[str]:
    ...
    if not stamp.is_file():
        problems.append("%s has no provenance stamp; run `homm1 build`" % output)
        return problems
    try:
        payload = json.loads(stamp.read_text())
    except (json.JSONDecodeError, OSError) as exc:
        problems.append("%s stamp is unreadable (%s); run `homm1 build`" % (output, exc))
        return problems
    if payload.get("schema") != STAMP_SCHEMA:
        problems.append("%s stamp has unknown schema; run `homm1 build`" % output)
        return problems
    for role, record in sorted(payload.get("inputs", {}).items()):
        ...
        if _sha256(input_path) != record.get("sha256"):
            problems.append(
                "%s is stale: %s input changed (%s); run `homm1 build`" %
