# Ported from scripts/homm2/analysis/string_xref.py at 299514f88900c0cf30ba03422c72830a38fc1cb7.
#!/usr/bin/env python3
"""homm1.navigation.string_xref - which string literals does a function reference?
...
Usage (reads $HOMM1_EXE + the CSVs only):
    python3 -m homm1.navigation.string_xref --rva 0x0000126d
    python3 -m homm1.navigation.string_xref --find "too many bits"
"""
...
def load():
    data = read_verified(targets()["game"], EXE)
    ...
    def func_of(rva):
        ...
        st = starts[i]
        sz = fsize.get(st)
        return st if sz and rva < st + sz else None
...
def main():
    args = sys.argv[1:]
    if not EXE.is_file():
        sys.exit(f"[string_xref] target EXE not found: {EXE} (set $HOMM1_EXE)")
