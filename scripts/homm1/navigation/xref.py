# Ported from scripts/homm2/analysis/xref.py at 299514f88900c0cf30ba03422c72830a38fc1cb7.
#!/usr/bin/env python3
"""homm1.navigation.xref - who calls this function? (retail call/jmp-graph xrefs).
...
attributes a leaf to its owning class/TU). This scans HEROES.exe's .text for direct
...
a known boundary. Generate it once with `homm1 ghidra`.

Unlike gruntz's GRUNTZ.EXE, HEROES.exe is NOT incrementally linked - there is no ILT
...
Usage (reads files + $HOMM1_EXE only):
    python3 -m homm1.navigation.xref 0x00069120 0x0006e0f0   # callers of each
    python3 -m homm1.navigation.xref ?SmackManagerMain@@YIXXZ # by (mangled) name
    python3 -m homm1.navigation.xref --callees 0x00069120     # forward: its call targets
    python3 -m homm1.navigation.xref --raw 0x00069120         # every call site (no dedup)
    python3 -m homm1.navigation.xref --tree 0x00069120        # caller ancestry (depth 4)
    python3 -m homm1.navigation.xref --tree --depth 0 0x...    # unlimited (can be huge)
"""
...
from homm1.core.inputs import targets, read_verified
EXE = targets()["game"].destination
...
def _load():
    d = read_verified(targets()["game"], EXE)
...
def _owner(rva, fstarts, fsize):
    ...
    gap. Nearest start <= rva, bounded by that fn's known size. Size unknown -> no attribution (HoMM1 sparse-census adaptation)."""
    k = bisect.bisect_right(fstarts, rva) - 1
    if k < 0:
        return None
    start = fstarts[k]
    sz = fsize.get(start)
    if not sz or rva >= start + sz:
        return None
    return start
...
def data_refs(target, d, secs, names):
    ...
    .text scan can't see. Returns [(data_rva, section)]. (No ILT thunks in HEROES, so
...
def main():
    ...
    if not rest:
        sys.exit(__doc__)
    if not EXE.is_file():
        sys.exit(f"[xref] target EXE not found: {EXE} (set $HOMM1_EXE)")
    d, secs = _load()
