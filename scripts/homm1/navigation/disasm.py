# Ported from scripts/homm2/analysis/disasm.py at 299514f88900c0cf30ba03422c72830a38fc1cb7.
#!/usr/bin/env python3
"""homm1.navigation.disasm - per-function disassembly of a HEROES function.
...
Usage:
    python3 -m homm1.navigation.disasm 0x000d4050              # target
    python3 -m homm1.navigation.disasm ?initialiseCRC@@YIXXZ --base
    python3 -m homm1.navigation.disasm 0x000d4050 --diff
"""
import csv, json, os, re, subprocess, sys, tempfile
from pathlib import Path

REPO = Path(os.environ.get("HOMM1_DIR")) if os.environ.get("HOMM1_DIR") else \
    next((p for p in Path(__file__).resolve().parents if (p / "flake.nix").exists()),
         Path(__file__).resolve().parents[3])
SYMCSV = REPO / "build/gen/symbol_names.csv"
DELINK = REPO / "build/objdiff/target"
BASE = REPO / "build/objdiff/base"
NORMAL_BASE = REPO / "build/objdiff/base"
NORMAL_TARGET = REPO / "build/objdiff/target"
...
def _resolve(arg):
    ...
    if not SYMCSV.is_file():
        die(f"{SYMCSV} missing - run `homm1 init`")
    want_rva = None
    try:
        want_rva = int(arg, 16)
    except ValueError:
...
def _objdump(obj: Path, name: str, size_hint: int, ordinal: int) -> str:
    if not obj.is_file():
        die(f"{obj.relative_to(REPO)} missing - "
            + ("run `homm1 build` first" if BASE in obj.parents else "run `homm1 init` first"))
    if NORMAL_TARGET in obj.parents:
    ...
    # VC4 /Z7 emits classic COFF .lf records whose values are line counts,
    # not code offsets. LLVM treats them as labels and can restart decoding
    # inside a call operand. Buka's VC6 objects do not have this problem.
    # Remove only those debug markers from a disposable display copy; prove
    # its section bytes and named relocation stream are unchanged.
    from homm1.core.coff import CoffObject
    original = CoffObject(obj.read_bytes())
    with tempfile.TemporaryDirectory(prefix='homm1-disasm-') as directory:
        display = obj
        if any(s.name in ('.lf', '.bf', '.ef') for s in original.symbols.values()):
            display = Path(directory) / obj.name
            stripped = subprocess.run(['llvm-objcopy', '--strip-symbol=.lf', '--strip-symbol=.bf', '--strip-symbol=.ef',
                                       str(obj), str(display)], capture_output=True, text=True)
            if stripped.returncode:
                die('cannot prepare VC4 disassembly view: ' + stripped.stderr)
            view = CoffObject(display.read_bytes())
            def payload(o):
                return ([o.section_bytes(s) for s in o.sections],
                        [(r.section, r.site, r.typ, o.symbols[r.symbol_index].name) for r in o.relocations])
            if payload(original) != payload(view):
                die('VC4 disassembly view changed section bytes or relocations')
        res = subprocess.run(
            ["llvm-objdump", "-dr", "--x86-asm-syntax=intel", str(display)],
            capture_output=True, text=True)
    if res.returncode != 0:
...
def _hint_branches(rva: int, name: str, unit: str) -> None:
    ...
    print(f"[but this function is {pct:.2f}%, not 100 - and this view MASKS address "
          ...
          f"`homm1 sema disasm 0x{rva:08x} --branches --diff`, which names each "
...
def _branch_view(rva: int, name: str, unit: str, size: int, ordinal: int,
    ...
    if not want_diff:
        ...
        obj = BASE / f"{unit}.obj" if want_base else DELINK / f"{unit}.obj"
    ...
    ti = _branch_insns(_objdump(NORMAL_TARGET / f"{unit}.obj", name, size, ordinal))
...
def main():
    ...
    if "--blocks" in flags:
        if "--diff" in flags:
            ...
            target_text = _objdump(
                NORMAL_TARGET / f"{unit}.obj", name, size, ordinal)
            base_cfg, target_cfg = _cfg(base_text), _cfg(target_text)
            if "--dot" in flags:
                import difflib
                base_rows = ["\n".join(body + [term or ""])
        ...
        obj = BASE / f"{unit}.obj" if use_base else DELINK / f"{unit}.obj"
    ...
    if "--diff" in flags:
        ...
        tgt = _norm(_objdump(NORMAL_TARGET / f"{unit}.obj", name, size, ordinal))
        if base == tgt:
    ...
    if "--base" in flags:
        ...
        print(f"[disasm TARGET (retail): {name}  build/delink/{unit}.obj]")
        text = _objdump(DELINK / f"{unit}.obj", name, size, ordinal)
