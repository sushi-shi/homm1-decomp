# Ported from scripts/homm2/core/wine.py at 299514f88900c0cf30ba03422c72830a38fc1cb7.
#!/usr/bin/env python3
"""homm1.core.wine - the era-toolchain wine plumbing, in one place.
...
def run_hang_proof(command: list[str], produces: Path, *, cwd: Path,
                   timeout: float | None = None, environment: dict | None = None) -> tuple[str, int, bool]:
    ...
    limit = timeout if timeout is not None else float(
        os.environ.get("HOMM1_CL_TIMEOUT", "300"))
    timed_out = False
    with tempfile.TemporaryFile() as logf:
        proc = subprocess.Popen(
            command, cwd=str(cwd), env=child_env() if environment is None else {**environment, 'TZ': 'UTC0'}, stdin=subprocess.DEVNULL,
