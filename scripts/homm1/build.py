...
def configure():
    ...
    dependencies += [REPO / 'build/environment.sha256']
...
def _run(args):
    ...
    dependencies = {str(p.relative_to(REPO)) if p.is_relative_to(REPO) else str(p): toolchain.digest(p)
                    for unit in entries for p in analysis.dependencies(REPO / unit['source'], compiler, config['flags'][unit['flags']])}
    publication.atomic_write(REPO / 'build/environment.sha256', generation + '\n')
    configure()
    ...
    namespace = {}
    internal_rvas = {c.rva for c in claims if c.linkage == 'internal'}
    for refs in references.values():
        for ref in refs:
            name, rva = ref['symbol'], ref['target_rva']
            if rva in internal_rvas:
                continue
    ...
    for unit in entries:
        ...
        delink.stamp_target(unit['unit'], target)
